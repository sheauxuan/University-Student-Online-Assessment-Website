...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['publish'])) {
    ...
    if (isset($_FILES['quiz_image']) && $_FILES['quiz_image']['error'] === 0) {
        if ($_FILES['quiz_image']['size'] > 5000000) { 
            echo "<script>alert('File is too large. Maximum allowed size is 5MB.'); window.location.href='module.php';</script>";
            exit();
        } else {
        ...
        if (mysqli_stmt_execute($stmt)) {
                    ...
                    if ($question_type == 'objective') {
                        ...
                        if ($stmt_question = mysqli_prepare($connection, $insert_question_query)) {
                            ...
                            if (!mysqli_stmt_execute($stmt_question)) {
                                echo "<script>alert('Error inserting question.'); window.location.href='createquiz.php.php';</script>";
                                exit();
                            }
                        ...
                        if ($stmt_question = mysqli_prepare($connection, $insert_question_query)) {
                            ...
                            if (!mysqli_stmt_execute($stmt_question)) {
                                echo "<script>alert('Error inserting question.'); window.location.href='createquiz.php';</script>";
                                exit();
                            }
                            mysqli_stmt_close($stmt_question);
                        }
            ...
            if ($activity_stmt = mysqli_prepare($connection, $activity_query)) {
                ...
                if (!mysqli_stmt_execute($activity_stmt)) {
                    echo "<script>alert('Error inserting activity summary.'); window.location.href='jiajia.php';</script>";
                    exit();
                }
                mysqli_stmt_close($activity_stmt);
            }

            echo "<script>alert('Quiz has been published successfully!'); window.location.href='createquiz.php';</script>";
            exit();
        } else {
            echo "<script>alert('Failed to publish quiz. Please try again.'); window.location.href='createquiz.php';</script>";
            exit();
        }
        mysqli_stmt_close($stmt);
    } else {
        echo "<script>alert('Database error: Unable to process your request.'); window.location.href='createquiz.php';</script>";
        exit();
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .input-group {
            margin-bottom: 20px; /* Add spacing between the input groups */
        }
    ...
    <main class="main">
        <div class="section">
            ...
            <div class="input-group quiz-image">
                ...
                <!-- Container for the first question and dynamically added questions -->
